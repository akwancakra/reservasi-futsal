...
vector<vector<string>> Account = {{"1", "akwan", "akwan@mail.com", "akwan", "2"}, {"2", "cakra", "cakra@mail.com", "cakra", "1"}}, Lapangans = {{"1", "Lapangan Hitam", "20000", "1"}, {"2", "Lapangan Normal", "25000", "1"}, {"3", "Lapangan Medium", "30000", "1"}}, Tools = {{"1", "Cone", "10000", "5", "100", "1"}, {"2", "Rompi", "10000", "5", "100", "1"}, {"3", "Bola", "4000", "1", "12", "1"}};
vector<vector<string>> Transactions = {{"1", "1", "1", "0", "0", "2", "1", "5000", "12:00", "14:30", "11", "12", "2022"}, {"2", "2", "1", "0", "0", "14", "1", "5000", "07:00", "21:00", "12", "12", "2022"}, {"3", "1", "1", "0", "0", "1", "1", "5000", "14:30", "15:30", "11", "12", "2022"}};
...
// GET JADWAL CALENDAR
void GetCalendar(int bulan = 1, int tahun = 2022, int mode = 2)
{
    // 14 OREN
    // 13 UNGU
    // 12 MERAH
    ...
    for (int i = 1; i <= sumDay + tab; i++)
    {
        ...
        else if (i > tab)
        {
            ...
            {
                if (stoi(Transactions[j][1]) == idAccount && mode == 1)
                {
                ...
                else if (stoi(Transactions[j][1]) == idAccount && mode == 2)
                {
                    if (stoi(Transactions[j][1]) == idAccount && stoi(Transactions[j][10]) == (i - tab) && stoi(Transactions[j][11]) == bulan && stoi(Transactions[j][12]) == tahun)
                    {
                        main += stoi(Transactions[j][5]);
                        if (j == Transactions.size() - 1)
                        {
...
void GetJam(int idLapangan, int dur, string tanggal, string bulan, string tahun, string &waktuAwal)
{
    ...
    for (int i = 0; i < 15; i++)
    {
        ...
        // if (calendar.waktuAll[i] != (float)0 || calendar.waktuAll[i] != (float)0.1 || calendar.waktuAll[i] != (float)0.2 || calendar.waktuAll[i] != (float)0.3 || calendar.waktuAll[i] != (float)0.4 || calendar.waktuAll[i] != (float)0.5)
        else
        {
    ...
    do
    {
        cout << "Pilih waktu waktu awal co(15:30, 12.00)\t: ";
        cin >> waktuAwal;
...
void getDetail(string lap, string tool, string jmlTool, string dur, string tanggal, string bulan, string tahun, string waktuMulai)
{
    ...
    for (int i = 0; i < Lapangans.size(); i++)
    {
        if (stoi(Lapangans[i][0]) == stoi(lap))
        {
            ...
            } while (isNumber(pay) != true || stoi(pay) != 1);
...
void CreateSewa()
{
    ...
    system("CLS");
    GetCalendar(stoi(bulan), stoi(tahun));
    do
    {
...
void ShowCalendar()
{
    ...
    system("CLS");
    GetCalendar(stoi(bulan), stoi(tahun), 2);
    system("pause");
    ShowCalendar();
}
